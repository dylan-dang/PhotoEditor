...
import static utils.DrawingHelper.drawBorderedArea;

public class ColorSelector extends JPanel {
    private JColorChooser primary, secondary;
    private Rectangle primaryArea = new Rectangle(), secondaryArea = new Rectangle(),
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (primaryArea.contains(mousePos)) { // click on primary square
                    JColorChooser
                            .createDialog((ColorSelector) e.getSource(),
                                    "Color Picker (Primary Color)", true, primary, null, null)
                            .setVisible(true);
                } else if (secondaryArea.contains(mousePos)) { // click on secondary square
                    JColorChooser
                            .createDialog((ColorSelector) e.getSource(),
                                    "Color Picker (Secondary Color)", true, secondary, null, null)
                            .setVisible(true);
    ...
    public void paintComponent(Graphics g) {
        ...
        drawChecker(g2, secondaryArea.x + 2, secondaryArea.y + 2, secondaryArea.width - 3,
                secondaryArea.height - 3, 4);
        drawBorderedArea(g2, secondaryArea, Color.black, Color.white, secondary.getColor());
        // draw the secondary color box
        drawChecker(g2, primaryArea.x + 2, primaryArea.y + 2, primaryArea.width - 3,
                primaryArea.height - 3, 4);
        drawBorderedArea(g2, primaryArea, Color.black, Color.white, primary.getColor());
        ...
        drawBorderedArea(g2, new Rectangle(3, 24, 7, 7), foreground, Color.white);
        drawBorderedArea(g2, new Rectangle(0, 21, 7, 7), foreground, Color.black);

        // draw the arrows
        g.setColor(foreground);
        Polygon arrows = new Polygon(new int[] {28, 23, 23, 21, 23, 23, 28, 28, 26, 28, 30, 28, 28},
                new int[] {3, 3, 1, 3, 5, 3, 3, 8, 8, 10, 8, 8, 3}, 13);
        g2.translate(0, -32 / getPreferredSize().height + .2); // translate when smaller b/c for
                                                               // some reason it looks misaligned
                                                               // when small
